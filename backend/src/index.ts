...
} from "./data/loops/phishermanLoops";
...
const SAFE_TO_IGNORE_ERIS_ERROR_MESSAGES = [
...
function errorHandler(err) {
	...
	if (err instanceof PluginLoadError) {
		// tslint:disable:no-console
		console.warn(
			`${guildName} (${guildId}): Failed to load plugin '${err.pluginName}': ${err.message}`
		);
		return;
	}
...
connect().then(async () => {
	...
	client.on(Constants.Events.DEBUG, (errorText) => {
		...
		if (value > safe429MaxCount) {
			// tslint:disable-next-line:no-console
			console.error(
				`Too many 429s (over ${safe429MaxCount} in ${
					safe429MaxCount * safe429DecayInterval
				}ms), exiting`
			);
			process.exit(1);
		}
		...
		options: {
			...
			async getEnabledGuildPlugins(ctx, plugins): Promise<string[]> {
				...
				return Array.from(plugins.keys()).filter((pluginName) => {
					...
					return (
						configuredPlugins[pluginName] &&
						configuredPlugins[pluginName].enabled !== false
					);
				});
			},
			...
			sendSuccessMessageFn(channel, body) {
				const guildId =
					channel instanceof TextChannel || channel instanceof ThreadChannel
						...
						: undefined;
				const emoji = guildId
					? bot.getLoadedGuild(guildId)!.config.success_emoji
					: undefined;
	...
	client.once("ready", () => {
		...
		client.user?.setPresence({
			activities: [{ name: "the waters", type: ActivityType.Watching }]
		});
	});
	...
	setInterval(() => {
		lowestGlobalRemaining = Math.min(
			lowestGlobalRemaining,
			(client as any).rest.globalRemaining
		);
	}, 100);
	setInterval(() => {
		// FIXME: Debug
		if (lowestGlobalRemaining < 30) {
			// tslint:disable-next-line:no-console
			console.log(
				"[DEBUG] Lowest global remaining in the past 15 seconds:",
				lowestGlobalRemaining
			);
		}
