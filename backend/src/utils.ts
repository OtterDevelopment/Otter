...
export const tNormalizedNullOrUndefined = new t.Type<undefined, null | undefined>(
	...
	(v, c) =>
		v == null ? t.success(undefined) : t.failure(v, c, "Value must be null or undefined"),
	(s) => undefined
);
...
export function differenceToString(diff: Map<string, { was: any; is: any }>): string {
	...
	for (const [key, difference] of diff) {
		toReturn += `**${key[0].toUpperCase() + key.slice(1)}**: \`${difference.was}\` ➜ \`${
			difference.is
		}\`\n`;
	}
	return toReturn;
...
export const tAlphanumeric = new t.Type<string, string>(
	...
	(from, to) =>
		either.chain(t.string.validate(from, to), (s) => {
			return s.match(/\W/)
				? t.failure(from, to, "String must be alphanumeric")
				: t.success(s);
		}),
	(s) => s
);
...
export const tDateTime = new t.Type<string, string>(
	...
	(from, to) =>
		either.chain(t.string.validate(from, to), (s) => {
			const parsed =
				s.length === 10
					? moment.utc(s, "YYYY-MM-DD")
					: s.length === 19
					? moment.utc(s, "YYYY-MM-DD HH:mm:ss")
					: null;

			return parsed && parsed.isValid()
				? t.success(s)
				: t.failure(from, to, "Invalid datetime");
		}),
	(s) => s
);
...
export function convertDelayStringToMS(str, defaultUnit = "m"): number | null {
	...
	while (str !== "" && (match = str.match(regex)) !== null) {
		ms +=
			match[1] *
			((match[2] && delayStringMultipliers[match[2]]) || delayStringMultipliers[defaultUnit]);
...
export function trimEmptyStartEndLines(str: string) {
	...
	return lines
		.slice(emptyLinesAtStart, emptyLinesAtEnd ? -1 * emptyLinesAtEnd : undefined)
		.join("\n");
}
...
export function resolveUserId(bot: Client, value: string) {
	...
	if (usernameMatch) {
		...
		const user = bot.users.cache.find(
			(u) => u.username === usernameMatch[1] && u.discriminator === usernameMatch[2]
		);
...
const internalStickerCache: LimitedCollection<Snowflake, Sticker> = new LimitedCollection({
	maxSize: 500
});
